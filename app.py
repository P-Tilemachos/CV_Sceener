...
Claude API with a structured prompt and returns:
  - a match score (0-100)
  - top missing keywords
  - red flags a human reviewer would notice first

This is a starter/reference implementation, meant to be extended.
...
# Config
# ---------------------------------------------------------------------
MODEL_NAME = "claude-sonnet-4-6"  # swap for whichever model you have access to
...
def build_prompt(cv_text: str, job_description: str) -> str:
    """Build the user prompt sent to the model. Keep instructions explicit
    and ask for JSON only, so the output is easy to parse deterministically."""
...
def call_claude(prompt: str, api_key: str) -> dict:
    """Send the prompt to the Claude API and parse the JSON response."""
...
st.caption(
    ...
    "Claude API and structured prompting."
)
...
with st.sidebar:
    ...
    api_key = st.text_input(
        "Anthropic API key",
        type="password",
        help="Get one at console.anthropic.com. Never commit this to GitHub.",
    )
    api_key = api_key or os.environ.get("ANTHROPIC_API_KEY", "")
    st.caption("Tip: set ANTHROPIC_API_KEY as an environment variable instead "
